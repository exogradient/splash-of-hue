// XKCD color survey — 949 human-sourced color names (CC0 public domain)
// Source: https://xkcd.com/color/rgb.txt
// Parsed at load time: hex → HSB conversion runs once (~1ms for 949 entries)
(function () {
  'use strict';
  var RAW = `
cloudy blue	#acc2d9
...
`.trim();
  ...
  function hexToHsb(hex) {
    ...
    return {
      h: +h.toFixed(1),
      s: +(max === 0 ? 0 : d / max * 100).toFixed(1),
      b: +(max * 100).toFixed(1),
    };
  }

  window.XKCD_COLORS = RAW.split('\n').map(function (line) {
    var parts = line.split('\t');
    var hsb = hexToHsb(parts[1]);
    return { name: parts[0], hex: parts[1], h: hsb.h, s: hsb.s, b: hsb.b };
  });
})();
