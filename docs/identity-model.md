...
| **Call It** | Idea | Encoding (vocabulary) | Picker, visual feedback (commit before seeing answer) | See color → pick its name from choices (cerulean, burnt sienna, etc.) → reveal correct name + HSB values. Builds color vocabulary |
| **Split It** | Idea | Encoding (parametric) | Picker, visual feedback (commit before seeing answer) | See color → classify into HSB range buckets (e.g. warm/cool, high/low sat) → reveal answer + color name. Difficulty scales from coarse buckets to narrow ranges |
